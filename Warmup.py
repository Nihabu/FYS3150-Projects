...
for i in range(len(steplength)):
    ...
    f2c[i] = (f(x_plus) - f(x))/steplength[i]
    f3c[i] = (f(x_plus) - f(x_minus))/(2*steplength[i])
    #f2c[i] = (arctan(sqrt(2)+steplength[i]) - arctan(sqrt(2)))/steplength[i]
    #f3c[i] = (arctan(sqrt(2)+steplength[i]) - arctan(sqrt(2)-steplength[i]))/(2*steplength[i])
...
plt.plot(lgsl, er2, label='er2')
plt.plot(lgsl, er3, label='er3')
plt.legend()
plt.show()
...
f.write('Log10(er2)            Log10(er3)\n')

for i in xrange(len(er2)):
    f.write('%-18s %s\n' %(str(er2[i]), str(er3[i])))
    #f.write(str(er3[i]))
    #f.write('\n')
f.close()